...
??? proof "Proof."
    Any field of characteristic $p > 0$ contains $\mathbb{F}_p$ as a subfield. If $K$ and $L$ are algebraically closed fields of characteristic $p$, then $K \equiv L$ by Theorem (Chevalley-Tarski) and Proposition (Consequences of QE)(1), which proves that $\text{ACF}_p$ is complete.
...
??? proof "Proof."
    (1)$\iff$(2)$\iff$(3) follows from Theorem (Completeness of $\text{ACF}_p$).
