...
??? proof "Proof"
    Consider the cofinite filter $\mathcal F_{cof}$. By the Existence Theorem, there exists an ultrafilter $\mathcal U$ extending $\mathcal F_{cof}$. For any finite set $X \subseteq I$, its complement $I \setminus X \in \mathcal F_{cof} \subseteq \mathcal U$. Because $\mathcal U$ is a proper filter, $X \notin \mathcal U$. Thus $\mathcal U$ does not contain any singletons, so it cannot be a principal filter.

## Ultraproducts

!!! definition "Definition"
    Let $I$ be an index set, $\mathscr L$ be a first-order language. For each $i \in I$, let $\bar{M}_i$ be an $\mathscr L$-structure. Let $\mathcal U$ be an ultrafilter on $I$.
    The set
...
!!! theorem "Theorem (Łoś's Theorem)"
    <a id="thm-los-s-theorem"></a>
    Let $I$ be an index set, and $\varphi(x_1, \cdots,x_n)$ be an $\mathscr L$-formula. Let $\mathcal U$ be an ultrafilter on $I$, and $\{\bar{M}_i\}_{i \in I}$ be a family of $\mathscr L$-structures. For any elements $f_1, \cdots, f_n \in \prod_{i\in I} M_i / \mathcal U$, where $f_k := [(f_k(i))_{i\in I}]$, we have:
...
??? proof "Proof"
    We proceed by induction on the complexity of the formula $\varphi$. For simplicity, let $[f_i]$ denote the equivalence class $f_i/\sim_{\mathcal{U}}$.
...
!!! theorem "Theorem (Compactness via Ultraproducts)"
    <a id="thm-compactness-via-ultraproducts"></a>
    Let $\Sigma$ be a set of $\mathscr{L}$-sentences. If for any finite subset $\Sigma_0 \subseteq_{fin} \Sigma$, there is a model $\mathcal{M}_{\Sigma_0} \models \Sigma_0$ (i.e., $\Sigma$ is finitely satisfiable), then $\Sigma$ has a model (i.e., there is $\mathcal{M} \models \Sigma$).

??? proof "Proof"
    It suffices to consider the case where $\Sigma$ is infinite (otherwise it is trivially true).
