...
**Definition.** We say that $T$ is an $\mathscr L$-theory has quantifier elimination if for any formula $\varphi$ there is a quantifier-free formula $\psi$ such that $T\models \varphi \leftrightarrow \psi$.

**Example.** In the Theory of Real Closed Fields, the quantifier $\exists x\,(ax^2+bx+c=0)$ is equivalent to a quantifier-free formula $b^2-4ac\ge 0$.

**Definition.** Let $T$ be an $\mathscr L$-theory, $\mathscr L' \supseteq \mathscr L$, Assume that
...
**Proposition.** Any formula is logically equivalent to a formula whose terms are of height $\le 1$.

Hint. We do inductions, $R(f(g(x))) \leftrightarrow \exists y \ y=g(x) \land R(f(y))$

**Definition.** $T$ is an $\mathscr L$-theory, $\mathscr L' \supseteq \mathscr L, T' \supseteq T$. $T'$ be an $\mathscr L'$-theory. We say $T'$ is a **conservative expansion** of $T$ if for any $\varphi$ over $\mathscr L$, $T\models \varphi$ iff $T'\models \varphi$.

**Proposition.** Let $T'$ be an expansion by definition of $T$, then $T'$ is conservative over $T$. Moreover any $\mathscr L'$-formula is equivalent to an $\mathscr L$-formula over $T'$.
...
**Theorem (QE test).** Let $T$ be an $\mathscr L$-theory, $n\ge 1$ be an natural number, $\varphi(x_1,\cdots,x_n)$ be $\mathscr L$-formula. The following conditions are equivalent:
...
**Remark.** When $n=0$ and $\varphi$ is a sentence, one may consider $\varphi$ as $\varphi(x)$ and apply the theorem to $\varphi(x)$ to find a quantifier-free formula $\psi(x)$ which is equivalent to $\varphi(x)$ in $T$. For instance, the theorem $\exists y \ y=y$ of $T$ is equivalent in $T$ to the formula $x=x$.
...
**Proof.** (1) This is a special case of the easy implication in Theorem (QE test). Indeed, any sentence $\varphi$ is equivalent in $T$ to a quantifier-free formula $\psi(x)$. Let $\bar A$ be a common substructure of $\bar M$ and $\bar N$. For any $a \in A$, one then has
