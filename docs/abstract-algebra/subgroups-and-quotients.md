...
1. Let $S\subseteq G$ and
   $C=\{gsg^{-1}\mid g\in G,\ s\in S\}$.
   Prove that $\langle C\rangle\trianglelefteq G$ and
   $\langle C\rangle=\langle\!\langle S\rangle\!\rangle$.
   Prove from the quotient universal property that a homomorphism
   $f:G\to K$ kills $S$ exactly when it factors uniquely through
   $G/\langle\!\langle S\rangle\!\rangle$. Apply this to the set of
   commutators and recover the universal property of abelianization.

2. Let $N\subseteq K\trianglelefteq G$, with $N\trianglelefteq G$, and
   let $H\leq G$. Construct the natural map $G/N\to G/K$, compute its
   kernel and image, and derive the third isomorphism theorem. Then restrict
   $G\to G/N$ to $H$, compute its kernel and image, and derive the second
   isomorphism theorem. Explain why both are instances of the first
   isomorphism theorem.

3. Let $G$ be finite, let $p$ be the smallest prime divisor of $|G|$,
   and let $H,K$ be distinct subgroups of index $p$. Prove that $H$ and
   $K$ are normal and neither contains the other; show that $HK=G$; then
   prove $G/(H\cap K)\cong C_p\times C_p$ and interpret the quotient as
   recording two independent $C_p$-valued invariants.

4. Let $N,M\trianglelefteq G$ and $\Phi(g)=(gN,gM)$. Prove that
   $NM\trianglelefteq G$ and $\ker\Phi=N\cap M$. Show that
   $\operatorname{im}\Phi$ consists exactly of the pairs in
   $G/N\times G/M$ having the same image in $G/NM$. Specialize to
   $G=\mathbb Z$, $N=m\mathbb Z$, $M=n\mathbb Z$ to recover the
   compatible non-coprime CRT and its modulus $\operatorname{lcm}(m,n)$.

5. Suppose $N,M\trianglelefteq G$, $N\cap M=\{e\}$, and $NM=G$.
   Prove that every $g\in G$ has a unique expression $g=nm$. Define the
   two coordinate projections, prove that they are homomorphisms, compute
   their kernels, and show that together they invert the multiplication
   isomorphism $N\times M\to G$. In
   $S_3=A_3\langle(12)\rangle$, determine which analogous coordinate
   projection is not a homomorphism and explain the failure through
   normality of kernels.

6. Prove that every simple abelian group is isomorphic to $C_p$ for some
   prime $p$. If $f:G\to A$ is surjective with $A$ simple and abelian,
   prove that $\ker f$ is a maximal normal subgroup of prime index. Prove
   the converse.
